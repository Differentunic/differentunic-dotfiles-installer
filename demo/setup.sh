...
# Differentunic Dotfiles Bootstrap Script
set -e
...
info "Starting Differentunic Dotfiles Setup..."
...
TEMP_DIR=$(mktemp -d -t differentunic-installer-XXXXXX)
info "Cloning Differentunic Dotfiles Installer into $TEMP_DIR..."

# 3. Clone and Install the App
git clone --depth=1 https://github.com/differentunic/differentunic-dotfiles-installer.git "$TEMP_DIR"
cd "$TEMP_DIR"

info "Installing Differentunic Dotfiles Installer to ~/.local/bin..."
make install
...
differentunic-dotfiles-installer --install https://raw.githubusercontent.com/differentunic/dotfiles/main/hyprland-dotfiles.dotinst
